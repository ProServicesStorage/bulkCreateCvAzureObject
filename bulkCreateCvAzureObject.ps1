...
Usage: ./bulkCreateCvAzureObject.ps1
#>
...
#### Upsates (3/29/2024 ####
<#
Changed column names to match output from getSaKeys
...
$Logfile = ".\bulkCreateCvAzureObject.log"

# Set variables for Plan Name and Access Nodes Group
$planName = "Silver"
$accessNodes = "Infrastructure"

# Set CommServe hostname
$commserve = "butters.lab"
...
$csv = Import-Csv -Path ".\sa_keys.csv"
WriteLog "Imported CSV file $csv"

# Create the credential for Azure File.

foreach ($line in $csv) {
    $saName = $line.StorageAccountName
    $saKey = $line.StorageAccountKey
    ...
    try {
        ...
        $credId = $response.id
        WriteLog "Created Credential for Azure File Backups: $response"
        Write-Host "Created Credential for Azure File Backups: $response"
    }
    catch {
